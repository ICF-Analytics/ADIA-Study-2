...
library(clogitL1)
cll1 <- with(sdat, clogitL1(iX, y, id, numLambda = 100
...
cfit2 <- clogit(y ~
                  + anysuprelative_3 + anysupnonfam_3 + resid_stab_3
                + strata(id)
                , data = sdat)
...
mean(is.na(sdat$anysuprelative_3))
mean(is.na(sdat$anysupnonfam_3))
mean(is.na(sdat$resid_stab_3))

models <- lapply(filled, \(x)
                 clogit(y ~
                          + anysuprelative_3 + anysupnonfam_3 + resid_stab_3
                        + strata(id)
                        , data = data.frame(y = sdat$y, id = sdat$id, x)
                 ))
